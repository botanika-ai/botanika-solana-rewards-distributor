...
async fn test_initialize_invalid_decimals() {
    ...
    context
        .create_mint(&reward_mint, &context.payer.pubkey(), 6)
        .await
        .unwrap();

    let (reward_distributor_pda, _) =
        Pubkey::find_program_address(&[RewardDistributor::SEED], &program_id);
    ...
    let result = context
        .process_transaction(&[initialize_ix], &[&token_vault])
        .await;
    assert!(result.is_err());
}
